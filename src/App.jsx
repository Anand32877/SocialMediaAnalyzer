...
//             {[
//               { q: "What is a Social Media Content Analyzer?", a: "It’s a tool that extracts text from PDFs or images and gives AI-powered summaries, insights, and engagement tips." },
// { q: "Which file formats can I upload?", a: "You can upload PDFs (text & scanned) and images like JPG, PNG, or WEBP." },
// { q: "Can it analyze captions and hashtags?", a: "Yes, it can extract text from images and suggest improved captions, hashtags, and CTAs." },
// { q: "Is an internet connection required?", a: "Yes, for AI-based features like summarization and engagement tips. Basic text extraction works offline." },
// { q: "Do I need an API key?", a: "Yes, set VITE_GEMINI_API_KEY in your environment for AI responses." },
// { q: "Is my uploaded data stored?", a: "No, your files are processed temporarily and not saved anywhere." },
...
//             <p className="mt-2 text-xs">© {new Date().getFullYear()} All Rights Reserved | Made with ❤️ by <b>Anand Mishra</b></p>
//           </div>
//         </footer>
//       </main>
//     </div>
...
export default function App() {
  ...
  const [progress, setProgress] = useState(null);
  const [faqOpen, setFaqOpen] = useState(null);
  // default = light as you requested
  ...
  // Footer adaptive classes (visible in all themes)
  const footerClasses =
    theme === "light"
  ...
  return (
    <div
      className={`min-h-screen ${themeClasses[theme]} transition-colors duration-500`}
    >
      {/* ---------- Header ---------- */}
      <header
        className={`w-full ${
          theme === "light" ? "bg-white/80" : "bg-black/10"
        } backdrop-blur-sm py-3 shadow`}
      >
        <div className="max-w-6xl mx-auto px-6 flex items-center justify-between">
          <div className="font-extrabold text-xl">📄 Social Media Content Analyzer</div>
          ...
          <div className="flex items-center gap-6">
            ...
            <div className={`${theme === "light" ? "text-black/70" : "text-white/80"} flex gap-6 text-sm`}>
              <a href="#how" className="hover:underline">How to Use</a>
      ...
      <main className="max-w-6xl mx-auto px-6 py-10">
        <section
          className={`rounded-xl p-10 text-center shadow-xl ${
            theme === "light"
              ? "bg-white"
              : theme === "dark"
              ? "bg-gray-800/60"
        ...
        >
          <div
            className="p-10 rounded-md border-2 border-dashed border-white/20 hover:bg-white/5 cursor-pointer"
            onClick={() => fileInputRef.current.click()}
          >
            <UploadCloud size={40} className={`mx-auto mb-4 ${theme === "light" ? "text-indigo-600" : "text-white"}`} />
            <h2 className={`text-2xl font-bold mb-2 ${theme === "light" ? "text-black" : "text-white"}`}>
              Upload PDF / Image
            </h2>
            <p className={`${theme === "light" ? "text-gray-600" : "text-white/90"} opacity-80 mb-4`}>
              Drag & drop or click here to choose your file
            </p>
            <input
              ref={fileInputRef}
              type="file"
          ...
          {progress && <p className={`mt-4 ${theme === "light" ? "text-black" : "text-white"}`}>{progress}</p>}
          {text && (
            <div className={`${theme === "light" ? "bg-white/90" : "bg-white/10"} mt-6 p-4 rounded-md text-left`}>
              <h3 className={`font-semibold mb-2 ${theme === "light" ? "text-black" : "text-white"}`}>Extracted Text</h3>
              <div className="max-h-60 overflow-auto bg-white p-3 rounded text-black text-sm">
                {text}
              </div>
              <div className="flex gap-3 mt-3">
                <button
                  ...
                  className="flex items-center gap-2 bg-indigo-600 px-4 py-2 rounded-md text-white"
                >
                  <Download size={16} /> Download
                </button>
                <button
                  onClick={() => navigator.clipboard.writeText(text)}
                  className="flex items-center gap-2 bg-slate-200 px-4 py-2 rounded-md text-black"
                >
                  <Copy size={16} /> Copy
                </button>
              </div>
        ...
        {/* ---------- AI Tools Section ---------- */}
        <section className={`mt-10 rounded-xl p-8 shadow ${theme === "light" ? "bg-white text-black" : "bg-black/20 text-white"}`}>
          <h3 className="font-bold text-xl mb-4">🤖 AI Tools</h3>
          <div className="flex gap-4 flex-wrap mb-4">
            <button
              onClick={() => handleAI("summary")}
              className="flex items-center gap-2 px-5 py-2 bg-indigo-600 text-white rounded-md"
            >
              <FileText size={16} /> Summary
            </button>
            <button
              onClick={() => handleAI("hashtags")}
              className="flex items-center gap-2 px-5 py-2 bg-emerald-600 text-white rounded-md"
            >
              <Hash size={16} /> Hashtags
            </button>
            <button
              onClick={() => handleAI("rewrite")}
              className="flex items-center gap-2 px-5 py-2 bg-yellow-400 text-black rounded-md"
            >
              <RefreshCcw size={16} /> Rewrite
            </button>
            <button
              onClick={() => handleAI("sentiment")}
              className="flex items-center gap-2 px-5 py-2 bg-pink-500 text-white rounded-md"
            >
              Sentiment
            </button>
            <button
              onClick={() => handleAI("engagement")}
              className="flex items-center gap-2 px-5 py-2 bg-cyan-500 text-white rounded-md"
            >
              Engagement
            </button>
          </div>
          <div className={`${theme === "light" ? "bg-slate-100 text-black" : "bg-white/5 text-white"} p-4 rounded-md min-h-[150px] text-sm`}>
            {loading ? "🤖 Generating AI Response..." : aiResult || "AI output will appear here."}
          </div>
        </section>

        {/* ---------- How to Use Section ---------- */}
        <section id="how" className={`mt-12 rounded-xl p-10 shadow text-center ${theme === "light" ? "bg-white text-black" : "bg-black/10 text-white"}`}>
          <h3 className="text-3xl font-bold mb-6">📌 How to Use</h3>
          <div className="grid md:grid-cols-4 gap-6">
            {[
              { step: "Step 1", title: "Upload", desc: "Choose PDF or Image file to extract text.", icon: <UploadCloud size={40} className={`mx-auto ${theme === "light" ? "text-indigo-600" : "text-white"}`} /> },
              { step: "Step 2", title: "Extract", desc: "Our hybrid engine reads text or applies OCR.", icon: <FileText size={40} className="mx-auto text-purple-600" /> },
              { step: "Step 3", title: "Analyze", desc: "Use AI tools to summarize, rewrite or hashtags.", icon: <RefreshCcw size={40} className="mx-auto text-pink-600" /> },
              { step: "Step 4", title: "Engage", desc: "Apply AI suggestions to boost your reach.", icon: <Hash size={40} className="mx-auto text-green-600" /> },
            ].map((c, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.2 }} className={`p-6 rounded-xl shadow text-center ${theme === "light" ? "bg-slate-50" : "bg-white/5"}`}>
                <div className="font-bold text-sm text-slate-500">{c.step}</div>
                {c.icon}
                <h3 className="font-semibold text-lg mt-3">{c.title}</h3>
                <p className="text-sm mt-2 text-slate-600">{c.desc}</p>
        ...
        {/* ---------- FAQ Section ---------- */}
        <motion.section id="faq" initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} className={`mt-12 rounded-2xl p-10 shadow ${theme === "light" ? "bg-white text-black" : "bg-black/10 text-white"}`}>
          <h3 className="text-3xl font-bold mb-8 text-center">❓ Frequently Asked Questions</h3>
          <div className="space-y-4 max-w-3xl mx-auto">
            {[
              { q: "What is a Social Media Content Analyzer?", a: "It’s a tool that extracts text from PDFs or images and gives AI-powered summaries, insights, and engagement tips." },
        ...
        {/* ---------- Footer ---------- */}
        <footer className="mt-12 text-center">
          <div className={`${footerClasses} py-6 rounded-xl max-w-6xl mx-auto`}>
            <h4 className="font-bold text-lg">🌐 Social Media Content Analyzer</h4>
            <p className="text-sm opacity-80">Helping you extract, analyze & improve content</p>
            <p className="mt-2 text-xs">© {new Date().getFullYear()} All Rights Reserved | Made with ❤️ by <b>Anand Mishra</b></p>
          </div>
